...
La suma antes mencionada la pagaré en esta ciudad, en las oficinas principales de TECNNITEL, el día '.$corte_dia[0].' de '.$this->spanishMes($corte_fecha[1]).' del año '.$corte_fecha[0]).'.');
    ...
    public function get_ordenes_cliente(Request $request)
    {
        $data = Ordenes::where('id_cliente',$request->id_cliente)->get();
        return Datatables::of($data)->addIndexColumn()
            ...
            ->addColumn('actividad',function($data){
                return $data->get_actividad->actividad;
            })
            ...
            ->addColumn('action', function($data){
                ...
                <a class="dropdown-item" href="'.route("ordenes.imprimir",$data->id).'" target="_blank">Reporte</a>';
                if(false){
                $btn.='<a class="dropdown-item" href="'.route("ordenes.edit",$data->id).'">Editar</a>';
                }else{
                $btn.='<a class="dropdown-item" href="'.route("cliente.ordenes.edit",[$data->id,$data->id_cliente]).'">Editar</a>';
                }
                $btn.='<a class="dropdown-item" href="#" onclick="eliminar('.$data->id.','.$data->id_cliente.')">Eliminar</a>
                <div class="dropdown-divider"></div>
                </div>
                </div>';
                return $btn;
    ...
    public function suspensiones_index($id){
        ...
        $cliente = Cliente::find($id);
        $nombre_cliente = $cliente->nombre;
        return view('suspensiones.index',compact('id_cliente','nombre_cliente'));
    }

    public function get_suspensiones_cliente(Request $request)
    {   
        $data = Suspensiones::where('id_cliente',$request->id_cliente)->get();
        return Datatables::of($data)->addIndexColumn()
            ->addColumn('cliente',function($data){
                return $data->get_cliente->nombre;
            })
            ...
            ->addColumn('action', function($data){
                ...
                $btn.='<a class="dropdown-item" href="#" onclick="suspender('.$data->id.','.$data->id_cliente.')">Suspender</a>';
                }
                $btn.='<div class="dropdown-divider"></div>
                ...
                $btn.='<a class="dropdown-item" href="'.route("cliente.suspensiones.edit",[$data->id,$data->id_cliente]).'">Editar</a>';
                }
                $btn.='<a class="dropdown-item" href="#" onclick="eliminar('.$data->id.','.$data->id_cliente.')">Eliminar</a>
                <div class="dropdown-divider"></div>
                </div>
