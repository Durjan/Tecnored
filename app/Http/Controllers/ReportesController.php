...
class ReportesController extends Controller
{
    ...
    public function descargar_excel($tipo_reporte,$tipo_pago,$id_cobrador,$fecha_i,$fecha_f){
        //return $tipo_reporte.'/'.$tipo_pago.'/'.$id_cobrador.'/'.$fecha_i.'/'.$fecha_f;
        //return Excel::download(new FacturaExport, 'Reporte_Facturas.xlsx');
      
        $fecha_inicio =  $this->format_fecha_excel(1, $fecha_i);
        $fecha_fin =  $this->format_fecha_excel(2, $fecha_f);
        return (new FacturaExport($tipo_reporte,$tipo_pago,$id_cobrador,$fecha_inicio,$fecha_fin,$fecha_i,$fecha_f))->download('Reporte_Facturas.xlsx');
   


    }
    private function format_fecha_excel($x,$date){
        if($x==1){
            $fch=explode("-",$date);
    ...
    private function facturas($fecha_i,$fecha_f,$tipo_reporte,$tipo_pago,$id_cobrador){
        ...
        $fpdf->Cell(20,10,utf8_decode('desde '.$fecha_i.' hasta '.$fecha_f));
        if($tipo_reporte==0){
            $facturas = Factura::where('id_sucursal',Auth::user()->id_sucursal)->where('anulada',0)->whereBetween('created_at',[$fecha_inicio,$fecha_fin])->orderBy('numero_documento', 'ASC')->get();
        }
        if($tipo_reporte==1){
            $facturas = Factura::where('id_sucursal',Auth::user()->id_sucursal)->where('anulada',1)->whereBetween('created_at',[$fecha_inicio,$fecha_fin])->orderBy('numero_documento', 'ASC')->get();

        }
        if($tipo_reporte==2){
            $facturas = Factura::where('id_sucursal',Auth::user()->id_sucursal)->whereBetween('created_at',[$fecha_inicio,$fecha_fin])->orderBy('numero_documento', 'ASC')->get();            
        }
        $fpdf->Ln();
        ...
        foreach($facturas as $row){
            ...
            if($tipo_pago==$row->tipo_pago OR $tipo_pago=="")
            {
                ...
                {   
                    ...
                    $fpdf->Cell(75,7,utf8_decode(substr($row->get_cliente->nombre,0,39)),0,0,'L');
