...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $cliente = Cliente::select('id')->where('activo',1)->where('internet',1)->get();
        $clientes=$cliente->count();
        $fecha_inicio = date('Y-m-d 00:00:00');
        $fecha_fin = date('Y-m-d 23:59:59');
        $factura = Factura::select('total')->where('anulada',0)->whereBetween('created_at',[$fecha_inicio,$fecha_fin])->get();
        $total_fac=0;
        foreach ($factura as $value) {
           $total_fac+=$value->total;
        }

        $cargos_pen_inte = Abono::select('abonos.id_cliente','abonos.pagado','abonos.anulado','clientes.id','clientes.id_sucursal')
        ...
        $cargos_pen_tv = Abono::select('abonos.id_cliente','abonos.pagado','abonos.anulado','clientes.id','clientes.id_sucursal')
                            ->join('clientes','abonos.id_cliente','=','clientes.id')
                            ...
                            ->where('clientes.id_sucursal',Auth::user()->id_sucursal)
                            ->count();
        $total_pen=$cargos_pen_inte + $cargos_pen_tv;
